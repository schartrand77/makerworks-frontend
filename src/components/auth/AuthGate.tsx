...
export default function AuthGate({ children, role = null }: AuthGateProps): JSX.Element | null {
  ...
  if (role && !user.groups?.includes(role)) {
    console.warn(`[AuthGate] User lacks required group "${role}". Redirecting to /unauthorized`)
