...
const Browse: React.FC = () => {
  ...
  const [filter, setFilter] = useState<'all' | 'withThumbnail'>('all');
  ...
  const filteredModels =
    models?.filter(model => {
      ...
      const matchesFilter =
        filter === 'all' || (filter === 'withThumbnail' && !!model.thumbnail_url);
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-6 justify-center">
        ...
        <select
          value={filter}
          onChange={(e) => setFilter(e.target.value as 'all' | 'withThumbnail')}
